...
class ConditionalFlowMatcher:
    ...
    def sample_noise_like(self, key, x):
        # return torch.randn_like(x)
        return jnp.random.normal(key, size=x.shape)
    

    def sample_location_and_conditional_flow(self, key, x0, x1, t=None, return_noise=False):
        """
        ...
        if t is None:
            ...
            t = jnp.random.uniform(subkey, shape=(x0.shape[0],))
        assert len(t) == x0.shape[0], "t has to have batch size dimension"

        key, subkey = jax.random.split(key)
        eps = self.sample_noise_like(key, x0)
...
class VariancePreservingConditionalFlowMatcher(ConditionalFlowMatcher):
    ...
    def compute_mu_t(self, x0, x1, t):
        ...
        """
        t = pad_t_like_x(t, x0)
        return jnp.cos(jnp.pi / 2 * t) * x0 + jnp.sin(jnp.pi / 2 * t) * x1
    ...
    def compute_conditional_flow(self, x0, x1, t, xt):
        ...
        del xt
        t = pad_t_like_x(t, x0)
        return math.pi / 2 * (jnp.cos(jnp.pi / 2 * t) * x1 - jnp.sin(jnp.pi / 2 * t) * x0)
