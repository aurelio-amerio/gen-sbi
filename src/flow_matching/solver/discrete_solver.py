...
class MixtureDiscreteEulerSolver(Solver):
    ...
    Args:
        ...
        source_distribution_p (Optional[Array], optional): Source distribution, must be of shape [vocabulary_size]. Required only when divergence-free term for the probability velocity is non-zero. Defaults to None.
    """
    ...
    def __init__(
        ...
        vocabulary_size: int,
        source_distribution_p: Optional[Array] = None,
    ):
        super().__init__()
        self.model = model
        self.path = path
        ...
        if source_distribution_p is not None:
            assert source_distribution_p.shape == (
                vocabulary_size,
    ...
    def sample(
        ...
        dtype_categorical: jnp.dtype = jnp.float32,
        ...
        **model_extras,
        ...
        Args:
            ...
            x_init (Array): The initial state.
            ...
            dtype_categorical (jnp.dtype): Precision to use for categorical sampler. Defaults to jnp.float32.
            time_grid (Array): The CTMC process is solved in the interval [time_grid[0], time_grid[-1]] and if step_size is None then time discretization is set by the time grid. Defaults to jnp.array([0.0,1.0]).
        ...
        Returns:
            Array: The sampled sequence of discrete values.
        """
        if not div_free == 0.0:
            assert (
        ...
        t_init_scalar = time_grid[0].item()
        t_final_scalar = time_grid[-1].item()
        ...
        if step_size is None:
            ...
            assert (
                t_final_scalar - t_init_scalar
            ) > step_size, f"Time interval [time_grid[0], time_grid[-1]] must be larger than step_size. Got a time interval [{t_init_scalar}, {t_final_scalar}] and step_size {step_size}."

            # Use JAX ceil instead of math.ceil
            n_steps = int(jnp.ceil((t_final_scalar - t_init_scalar) / step_size))
            t_discretization_list = [t_init_scalar + step_size * i for i in range(n_steps)] + [t_final_scalar]
            t_discretization = jnp.array(t_discretization_list)

            if return_intermediates:
                # get order of intermediate steps:
                order = jnp.argsort(time_grid)
        ...
        current_t_val = t_init_scalar

        if verbose:
            ctx = tqdm(total=t_final_scalar, desc=f"NFE: {steps_counter}")
        
        for i in range(n_steps):
            key, key_model_sample, key_jump_rand, key_categorical_jump = jax.random.split(key, 4)
            
            t = t_discretization[i : i + 1]
            h = t_discretization[i + 1 : i + 2] - t_discretization[i : i + 1]

            # Sample x_1 ~ p_1|t( \cdot |x_t)
            t_for_model = jnp.full((x_t.shape[0],), t[0]) if x_t.ndim > 0 else t[0]
            p_1t = self.model(x=x_t, t=t_for_model, **model_extras)
            x_1 = categorical(key_model_sample, p_1t.astype(dtype_categorical))
            ...
            if i == n_steps - 1:
                ...
                delta_1 = jax.nn.one_hot(x_1, num_classes=self.vocabulary_size, dtype=k_t.dtype)
                ...
                if div_free_t > 0:
                    p_0_shape = (1,) * x_t.ndim + (self.vocabulary_size,)
                    p_0 = self.source_distribution_p.reshape(p_0_shape)
                    u = u + div_free_t * d_k_t / (k_t * (1 - k_t)) * (
                        (1 - k_t) * p_0 + k_t * delta_1
                    )

                # Set u_t(x_t|x_t,x_1) = 0
                delta_t = jax.nn.one_hot(x_t, num_classes=self.vocabulary_size)
                u = jnp.where(
                    delta_t.astype(jnp.bool_), jnp.zeros_like(u), u
                )

                # Sample x_t ~ u_t( \cdot |x_t,x_1)
                intensity = u.sum(axis=-1)  # Assuming u_t(xt|xt,x1) := 0
                mask_jump = jax.random.uniform(
                    key_jump_rand, shape=x_t.shape
                ) < 1 - jnp.exp(-h * intensity)

                x_t_if_jump = categorical(
                    key_categorical_jump, u.astype(dtype_categorical)
                )
                x_t = jnp.where(mask_jump, x_t_if_jump, x_t)

            steps_counter += 1
            current_t_val = t[0].item() + h[0].item()

            if return_intermediates and jnp.any(jnp.isclose(current_t_val, time_grid)):
                res.append(jnp.copy(x_t))
        ...
        if return_intermediates:
            if step_size is None:
                ...
                return jnp.stack(res, axis=0)[order]
        else:
            return x_t